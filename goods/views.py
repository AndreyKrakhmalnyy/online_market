...
def catalog(request, category_slug=None):
    ...
    """
    # Переменные ниже используются для обработки параметров get-запросов, получаемых из URL-строки, и извлекают значения передаваемых параметров (page, on_sale, order_by)
    ...
    #Если 'query'=True, то отобразить все товары из БД, иначе отображает по выбранному значению 'slug'. - описать понятнее
    elif query:
    ...
    #Если 'order_by'=True, то есть если галочка в фильтре сортировки указана и не равна 'default', то отсортировать товары.
    if order_by and order_by != "default":
        goods = Products.objects.order_by(order_by)

    paginator = Paginator(goods, 3) # класс Paginator принимает список товаров, которые удовлетворяют условиям выше и делит их на части, отображая 3 товара на странице.
    current_page = paginator.page(int(page)) # Получение текущей страницы url-строки get-запроса.
    context = {
        "title": "Home - Каталог",
        "goods": current_page,
